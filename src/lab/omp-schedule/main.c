...
void do_static(const int* vin, int* vout, int n) {
  /* [TODO] parallelize the following loop, simulating a
     ...
     STRIDE = num_threads * chunk_size;
     for (int my_block_start = START; my_block_start < n; my_block_start +=
     STRIDE) { for (int i = my_block_start; (i < my_block_start+chunk_size) &&
     (i < n); i++) { loop body
       }
     }
...
void do_dynamic(const int* vin, int* vout, int n) {
  /* [TODO] parallelize the following loop, simulating a
     ...
     - fetches the current value of `shared_idx` into a local (private) variable
     `my_block_start`;
