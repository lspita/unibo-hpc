 ...
 *
 * Copyright (C) 2017--2025 by Moreno Marzolla <https://www.moreno.marzolla.name/>
...
void init( float p )
{
    for (int i=TOP; i<=BOTTOM; i++) {
        for (int j=LEFT; j<=RIGHT; j++) {
            grid[cur][i][j] = (rand()/(double)RAND_MAX < p);
        }
    }
}
