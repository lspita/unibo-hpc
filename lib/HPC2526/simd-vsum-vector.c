 ...
 *
 * If you want to examine the generated object code, compile with:
 *
 *      gcc -O2 -std=c99 -Wall -Wpedantic -march=native -g -ggdb simd-vsum-vector.c -lm -o simd-vsum-vector
 *
 * and then
 *
 *      objdump -dS simd-vsum-vector
 *
 ****************************************************************************/
...
int main( int argc, char *argv[] )
{
    ...
    /* On some platforms, the compiler seems to emit SIMD aligned
       loads in vsum_vector; those loads fail if the vector is not
       properly aligned.  malloc() guarantees that the result is
    ...
    /* Under the GNU C library, posix_memalign() is defined in such a
       way that the compiler generates a warning if the return value
       is ignored (as we do). Assigning the return value to a dummy
       variable might trigger another warning ("unused variable XXX"),
       so a popular solution is to store the return value into `ret`,
